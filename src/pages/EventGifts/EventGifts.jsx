...
import { EventGifts } from "../../core/services/gift.service";
import { useParams } from "react-router-dom";
const MyEventGifts = () => {

  
  const [amountDetails, setAmountDetails] = useState({ totalGiftAmount: 0,totalGifts: 0,totalSentGiftAmount:0,totalSentGifts:0 });
 ...
 const [search, setSearch] = useState("");
  useEffect(() => {
    ...
    const fetchEventGifts = async (id)=>{
      try {
        const response = await EventGifts(id);
        setEventGifts(response)
      } catch (err){
        console.log(err)
      }
    }
    ...
    console.log("moin latif dddd",eventGifts)
  }, [id])
  return (
